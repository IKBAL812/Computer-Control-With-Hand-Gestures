...
class CameraApp:
    ...
    def handle_hand_landmarks(self, frame, hand_landmarks):
        ...
        if cy <= self.gestureThreshold:
            if cropped_hand.size != 0:
                ...
                if confidence > 0.8:
                    self.notify_command_triggered(hand_sign)  # Add this line to notify
    ...
    # Notify user about the triggered command
    def notify_command_triggered(self, command_name):
        notification.notify(
            title="Command Triggered",
            message=f"Triggered command = '{command_name}'",
            app_name="Gesture Control App",
